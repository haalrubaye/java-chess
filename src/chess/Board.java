...
public class Board {
	...
	public void capture(String attacker, String target) {
		int id = get(target).getId();
		put(target ,get(attacker));
		put(attacker, new EmptyPiece(id));
	}
	
	//
	public boolean isMoveInvalid(String move, char color) {
		String[] Move = move.split("-->");
		
		if(get(Move[0]).getColor() != color) return true;
		
		if (Arrays.asList(get(Move[0]).getPossibleMoves(this)).indexOf(Move[1]) == -1) {
			return true;	
		}
		ChessPiece temp = get(Move[1]);
		capture(Move[0], Move[1]);
		
		
		if (isCheck(Miscellanous.colorFlip(color), color)) {
			put(Move[0], temp);
			swap(Move[0], Move[1]);
			return true;
		}
	...
	public boolean isCheckmate(char attackerColor, char targetColor) {
		
		if(isCheck(attackerColor, targetColor)) {
			ChessPiece oppKingPiece = targetColor == 'W' ? get(getPosition(5)) : get(getPosition(61));
	...
	public boolean isStalemate(char attackerColor, char targetColor) {
	...
	public boolean isCheck(char attackerColor, char targetColor) {
		
		String oppKingPosition = targetColor == 'W' ? getPosition(5) : getPosition(61);
		for(ChessPiece piece: board.values()) {
			if(piece.getColor() == attackerColor) {
				System.out.println("Checkpoint 1");
				if(Arrays.asList(piece.getPossibleMoves(this)).indexOf(oppKingPosition) == -1){
					return true;
